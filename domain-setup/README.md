...
**Cause:** Invalid or missing ngrok auth token

**Solution:**
```bash
# Verify token in .env
cat ../.env | grep NGROK_AUTH_TOKEN

# Update if needed
echo "NGROK_AUTH_TOKEN=your_token_here" >> ../.env
```

### Issue: "domains.json not created"

**Cause:** Tunnel setup timed out or failed

**Solution:**
```bash
# Check ngrok terminal for errors
# Try manual tunnel creation
cd code
npm start

# Wait for "domains.json has been generated" message
```

### Issue: "Port already in use"

**Cause:** Another process is using ports 8080, 8081, or 3001

**Solution:**
```bash
# Find processes using the ports
lsof -i :8080 -i :8081 -i :3001

# Kill conflicting processes or stop existing tunnels
```

### Issue: "Tunnels disconnect frequently"

**Cause:** Network instability or ngrok rate limits

**Solution:**
- Check internet connection stability
- Consider upgrading ngrok plan for better reliability
- Use ngrok reserved domains (paid feature)

### Issue: "Services can't connect through tunnels"

**Cause:** Tunnels stopped or configurations not updated

**Solution:**
```bash
# Verify tunnels are running
cat code/domains.json

# Re-run setup to update configurations
./setup.sh
```
