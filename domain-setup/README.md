...
└────────┬─────────┘
    ...
    ├─────────────────────────────────┤
    │  • Issuer Portal (8080)         │
    │  • Verifier Portal (8081)       │
    │  • Trust Registry UI (3001)     │
    └─────────────────────────────────┘
```
...
└── code/
    ...
    ├── server.js         # Tunnel server (creates 3 tunnels)
    ├── generate.js       # One-time tunnel generator
    └── domains.json      # Generated tunnel URLs (created by scripts)
```
...
# Enable Keycloak tunnels for physical device testing (optional)
ENABLE_KEYCLOAK_TUNNELS=false
```

### Keycloak Tunnels for Physical Device Testing

If you're testing Keycloak authentication with a physical mobile device, you need additional tunnels:
...
2. **Start domain-setup**:
   ```bash
   ./domain-setup/setup.sh
   ```

3. **Configure Keycloak services** with ngrok URLs:
   ```bash
...
4. **Access from physical device**:
   - Open the Keycloak Demo App URL shown in domains.json
   - The QR code will work with the Ayra mobile app
...
```json
{
  ...
  "keycloak": {
    "demoApp": {
      "domain": "jkl012.ngrok-free.app",
      "url": "https://jkl012.ngrok-free.app"
    },
    "server": {
      "domain": "mno345.ngrok-free.app",
      "url": "https://mno345.ngrok-free.app"
    },
    "oidcBridge": {
      "domain": "pqr678.ngrok-free.app",
      "url": "https://pqr678.ngrok-free.app"
    }
  }
}
```

**Note:** The `keycloak` section only appears when `ENABLE_KEYCLOAK_TUNNELS=true`.
...
```bash
# Terminal will show:
🚀 Starting ngrok tunnels for Ayra services...
...
📝 domains.json has been generated

⚠️  Keep this terminal open to maintain the tunnels
