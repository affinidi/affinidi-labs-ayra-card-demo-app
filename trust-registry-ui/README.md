...
```bash
docker compose ps trust-registry-ui
docker compose logs trust-registry-ui
```

**Common causes:**
- Port 3001 already in use
- Build failed during Docker creation
...
# Rebuild container
docker compose up -d --build trust-registry-ui
```
...
# Check registries.ts configuration
cat code/src/data/registries.ts | grep apiEndpoint
```

**Solution:**
```bash
# Update API endpoint in registries.ts
# Rebuild and restart
docker compose restart trust-registry-ui
```

### Issue: "Issuers not displaying"

**Check:**
1. Trust Registry API is running
2. API endpoint is correct in `registries.ts`
3. CORS is properly configured
4. Issuer DIDs are properly URL-encoded
...
**Cause:** Special characters not properly URL-encoded

**Solution:**
Ensure colons in DIDs are encoded as `%3A`:
```
Wrong: did:web:example.com:8080:org
Right: did:web:example.com%3A8080:org
```
